...
namespace DuiLib
{
	...
	CDuiString::CDuiString() : m_pStr(m_szBuffer), m_nSize(MAX_LOCAL_STRING_LEN)
	{
		m_szBuffer[0] = '\0';
	}

	CDuiString::CDuiString(const TCHAR ch) : m_pStr(m_szBuffer), m_nSize(MAX_LOCAL_STRING_LEN)
	{
		m_szBuffer[0] = ch;
		m_szBuffer[1] = '\0';
	}

	CDuiString::CDuiString(LPCTSTR lpsz, int nLen) : m_pStr(m_szBuffer), m_nSize(MAX_LOCAL_STRING_LEN)
	{      
	...
	CDuiString::CDuiString(const CDuiString& src) : m_pStr(m_szBuffer), m_nSize(MAX_LOCAL_STRING_LEN)
	{
		m_szBuffer[0] = '\0';
		Assign(src.m_pStr);
	}
	...
	int CDuiString::EnsureSize(int newSize)
	{
		if ((unsigned)newSize <= m_nSize)
			return 0;
		TCHAR* newBuffer = static_cast<LPTSTR>(malloc(newSize * sizeof(TCHAR)));
		assert(newBuffer != 0);
		if (newBuffer == 0) {
			assert(false);
			return -1;
		}
		memcpy(newBuffer, m_pStr, m_nSize * sizeof(TCHAR));
		if (m_pStr != m_szBuffer) free(m_pStr);
		m_nSize = newSize;
		m_pStr = newBuffer;
		return 0;
	...
	void CDuiString::Assign(LPCTSTR pstr, int cchMax)
	{
		...
		if( cchMax + 1 < MAX_LOCAL_STRING_LEN && m_pStr != m_szBuffer) {
			free(m_pStr);
			m_pStr = m_szBuffer;
			m_nSize = MAX_LOCAL_STRING_LEN;
		}
		if (EnsureSize(cchMax + 1) < 0)
			return;
	...
	void CDuiString::Delete(int pos, int count)
	{
		std::wstring str(GetData());
		str.erase(pos, count);
		*this = str.c_str();
	...
	void CDuiString::Insert(int pos, LPCTSTR pstr)
	{
		std::wstring str(GetData());
		str.insert(pos, pstr);
		*this = str.c_str();
    ...
    int CDuiString::Format(LPCTSTR pstrFormat, va_list Args)
    {
		...
		if (EnsureSize(nLen + 1) < 0)
			return 0;
        nLen = _vsntprintf(m_pStr, nLen + 1, pstrFormat, Args);
		m_pStr[nLen] = 0;
        return nLen;
