	...
	void CVerticalLayoutUI::SetPos(RECT rc)
	{
		...
		for( int it2 = 0; it2 < m_items.GetSize(); it2++ ) {
			...
			if( sz.cy == 0 ) 
			{
				...
				int nIncHeight = pControl->GetAutoIncHeight();
				if (cyExpand > 0 && nIncHeight > 0)
					sz.cy = cyExpand + nIncHeight;
				else
					sz.cy = cyExpand;

				if (cyExpand > 0 && nIncHeight > 0 && (nAdjustables - iAdjustable) > 0)
				{//控件有额外需要增加的高度重新计算剩下区域的控件平均高度;
					cyExpand = MAX(0, (szAvailable.cy - cyFixed - sz.cy) /(nAdjustables- iAdjustable));
				}
...
//			cyFixedRemaining -= m_iChildPadding;
	...
	void CVerticalLayoutUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		else if (_tcscmp(pstrName, _T("visiblescroll")) == 0) SetVisibleScroll(_tcscmp(pstrValue, _T("true")) == 0);
		else if (_tcscmp(pstrName, _T("onlypage")) == 0) SetOnlyPage(_tcscmp(pstrValue, _T("true")) == 0);
		else if (_tcscmp(pstrName, _T("bkcoverimage")) == 0) SetBkCoverImage(pstrValue);
		else CContainerUI::SetAttribute(pstrName, pstrValue);
	}

	void CVerticalLayoutUI::SetVisibleScroll(bool bVisible)
	{
	...
	bool CVerticalLayoutUI::GetVisibleScroll()
	{
		return m_bVisibleScroll;
	}

	void CVerticalLayoutUI::SetOnlyPage(bool bOnlyPage)
	{
		m_bOnlyPage = bOnlyPage;
	}

	bool CVerticalLayoutUI::GetOnlyPage()
	{
		return m_bOnlyPage;
	}
	...
	void CVerticalLayoutUI::SetDrawBk(bool bDrawBk)
	{
		m_bDrawBk = bDrawBk;
		Invalidate();
	}
	...
	void CVerticalLayoutUI::PaintBkImage(HDC hDC)
	{
		if (!m_bDrawBk)
			return;
	...
	void CVerticalLayoutUI::PaintBkConverImage(HDC hDC)
	{
		if (m_sBkCoverImage.IsEmpty()) return;
		if (!DrawImage(hDC, (LPCTSTR)m_sBkCoverImage)) m_sBkCoverImage.Empty();
	}

	void CVerticalLayoutUI::DoPaint(HDC hDC, const RECT& rcPaint)
	{
		RECT rcTemp = { 0 };
			...
			if (!::IntersectRect(&rcTemp, &rcPaint, &rc))
			{
				for (int it = 0; it < m_items.GetSize(); it++) 
				{
					...
					if (pControl->IsFloat())
					{
						...
						if (m_pPrePaintCallback) m_pPrePaintCallback->OnPrePaint(pControl, it);
		...
		if (m_pPrePaintCallback) m_pPrePaintCallback->OnEndPaint();
