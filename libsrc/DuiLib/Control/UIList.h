...
{
public:
    virtual CDuiString GetItemText(CControlUI* pList, int iItem, int iSubItem, UINT &uTextStyle, DWORD &dwTextColor) = 0;
};
...
public:
    ...
    virtual bool SelectItem(int iIndex, bool bTakeFocus = false,bool bsendmsg=false) = 0;
    virtual void DoEvent(TEventUI& event) = 0;
	virtual void PaintGribLine(HDC hDC) = 0;
};
...
public:
    ...
    virtual void DrawItemText(HDC hDC, const RECT& rcItem, bool bParent=false) = 0;
};
...
public:
	...
	void SetPrePaintCallback(IItemPrePaintCallback* pCallback);
	...
	void SetSinglebkColor(DWORD dcolor);
	void SetDoublebkColor(DWORD dcolor);
	void SetVisibleScroll(bool bVisible);
	bool GetVisibleScroll();
	void SetItemHeight(int nheight);
	int GetItemHeight();
	void SetAlternatelyBk(bool balternately);
	void SetVisibleGridLine(bool bVisible);
	bool IsVisibleGridLine();
	void SetGridLineColor(DWORD dwColor);

protected:
    bool m_bScrollSelect;
	bool m_bVisibleGridLine;
    int m_iCurSel;
	int m_iCurSelActivate;  // 双击的列
    int m_iExpandedItem;
	DWORD m_singlebkcolor;
	DWORD m_doublebkcolor;
	DWORD m_dwGribLineColor;
	bool  m_bAlternatelybk;
	int	  m_nItemHeight;
...
public:
	...
	void SetLabelImage(LPCTSTR pstrImage);
public:
	CDuiRect m_rcTextPadding;
	DWORD m_dwDefTextColor;
	CDuiString m_strLabelImage;
};
...
public:
    ...
    void SetVisible(bool bVisible = true);
    void SetEnabled(bool bEnable = true);
	void SetVisibleSendMsg(bool bSend);
	...
	void SetEnableDrawBk(bool bEnableDrawBk);
    ...
    virtual void DrawItemText(HDC hDC, const RECT& rcItem, bool bParent = false);
...
protected:
    int m_iIndex;
	bool m_bVisibleSendMsg;
	bool m_bEnableDrawBk;
    bool m_bSelected;
	bool m_bMouseEvent;
    UINT m_uButtonState;
    IListOwnerUI* m_pOwner;
...
class UILIB_API IlistContainerElementCall
{
public:
	virtual void ListContainerDoEvent(CControlUI* pControl, TEventUI& event) = 0;
};
...
public:
	void SetICallBack(IlistContainerElementCall* pListCallBack);

private: 
	IlistContainerElementCall* m_pListCallBack;
};

class UILIB_API CListContainerElementComboUI : public CListContainerElementUI
{
public:
	CListContainerElementComboUI();
