...
{
    DUITRACE(_T("AX: CActiveXCtrl::GetContainer:%d"), GetTickCount());
...
{
    DUITRACE(_T("AX: CActiveXCtrl::ShowObject:%d"), GetTickCount());
    if( m_pOwner == NULL ) return E_UNEXPECTED;
...
{
    DUITRACE(_T("AX: CActiveXCtrl::OnShowWindow:%d"), GetTickCount());
    return E_NOTIMPL;
}
