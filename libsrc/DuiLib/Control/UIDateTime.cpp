...
#define DT_KEEP   3
	...
	void CDateTimeWnd::Init(CDateTimeUI* pOwner)
	{
		...
		if (m_hWnd == NULL)
		{
			...
			if(m_pOwner->IsTimerFormat())
				Create(m_pOwner->GetManager()->GetPaintWindow(), NULL, uStyle | DTS_UPDOWN | DTS_TIMEFORMAT, 0, rcPos);
			else
	...
	bool CDateTimeUI::IsTimerFormat()
	{
		return m_bTimerFormat;
	}
	...
	void CDateTimeUI::UpdateText()
	{
		...
		else if (m_nDTUpdateFlag == DT_UPDATE)
		{
			...
			if (m_pWindow != NULL) m_pManager->SendNotify(this, DUI_MSGTYPE_TEXTCHANGED);
		}
	}

	void CDateTimeUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		if (_tcscmp(pstrName, _T("timerformat")) == 0)
		{
