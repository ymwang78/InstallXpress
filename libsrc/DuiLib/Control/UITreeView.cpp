...
namespace DuiLib
{
	CTreeNodeUI::CTreeNodeUI( CTreeNodeUI* _ParentNode /*= NULL*/ )
	{
		...
		pTreeView		= NULL;
		m_bIsVisable	= false;  //edit 2014.11.7,应该为false,原来为true
		m_bIsCheckBox	= false;
	...
	bool CTreeNodeUI::AddChildNode( CTreeNodeUI* _pTreeNodeUI )
	{
		...
		if(nRet)
		{
			_pTreeNodeUI->SetVisible(GetFolderButton()->IsSelected());    //add 2014.11.7
	...
	bool CTreeViewUI::OnCheckBoxChanged( void* param )
	{
		...
		if(pMsg->sType == _T("selectchanged"))
		{
			...
			if(pItem->GetParentNode() != NULL)  //edit 2014.12.11
	...
	bool CTreeViewUI::OnFolderChanged( void* param )
	{
		...
		if(pMsg->sType == _T("selectchanged"))
		{
			...
			pItem->SetVisibleTag(pFolder->GetCheck());  //edit  2014.8.8
...
//			pItem->SetVisibleTag(pFolder->GetCheck());		//edit 这两行代码无用，pFolder->Selected(!pFolder->IsSelected()); 会触发OnFolderChanged函数，OnFolderChanged函数会执行这两行代码
//			SetItemExpand(pFolder->GetCheck(),pItem);
			return true;
		}
		return false;
