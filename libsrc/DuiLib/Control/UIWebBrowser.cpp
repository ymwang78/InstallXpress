...
bool DuiLib::CWebBrowserUI::DoCreateControl()
{
	...
	if (m_pManager) m_pManager->SendNotify(this, DUI_MSGTYPE_WEBCREATER, 0, 0, true);
	return true;
}
...
STDMETHODIMP DuiLib::CWebBrowserUI::ShowContextMenu( DWORD dwID, POINT* pptPosition, IUnknown* pCommandTarget, IDispatch* pDispatchObjectHit )
{
	if (m_pWebBrowserEventHandler)
	{
		return m_pWebBrowserEventHandler->ShowContextMenu(dwID,pptPosition,pCommandTarget,pDispatchObjectHit,this);
	}
	return S_FALSE;
}
...
void DuiLib::CWebBrowserUI::NavigateUrl( LPCTSTR lpszUrl )
{
	...
	{
		BSTR bstr = T2BSTR(lpszUrl);
		m_pWebBrowser2->Navigate(bstr/*(BSTR)SysAllocString(T2BSTR(lpszUrl))*/,NULL,NULL,NULL,NULL);
		::SysFreeString(bstr);
	}
}

void DuiLib::CWebBrowserUI::ExecScript(LPCTSTR lpscript)
{
	if (_pHtmlWnd2 == NULL)
	{
		IDispatch* pDp = NULL;
	...
	CComBSTR jscontent = SysAllocString(lpscript);	
	CComBSTR bstrlan = SysAllocString(_T("javascript"));
	VARIANT varRet;
	_pHtmlWnd2->execScript(jscontent, bstrlan, &varRet);
	::SysFreeString(jscontent);
