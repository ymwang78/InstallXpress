...
namespace DuiLib
{
	...
	//此方法不能使用定时器播放图片;
	void CGifAnimUI::UpdateGift(LPCTSTR pStrImage)
	{
		if (NULL == pStrImage)
		{
			DeleteGif();
			if (IsVisible()) this->Invalidate();
			return;
		}
		...
		if (m_pGifImage == NULL || m_pPropertyItem == NULL)
		{
			if (IsVisible()) this->Invalidate();
			return;
		}
		if( m_sBkImage.Right(4) == _T(".gif"))
		{
	...
	{
		if (m_bIsPlaying || m_pGifImage == NULL || m_pPropertyItem== NULL || m_pPropertyItem->length == 0 
			|| m_pPropertyItem->type>10 || m_pPropertyItem->value == NULL || m_sBkImage.Right(4) != _T(".gif"))
		{
			return;
		}
