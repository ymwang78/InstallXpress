...
namespace DuiLib
{
	...
	void CLabelUI::DoEvent(TEventUI& event)
	{
		...
		{
			if (m_pManager != NULL && m_bMouseEvent && IsEnabled()) {
	...
	{
		return m_bHandCur ? UIFLAG_SETCURSOR : __super::GetControlFlags();
	}
	...
	void CLabelUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		else if (_tcscmp(pstrName, _T("mouseevent")) == 0)
		{
			SetEnabledMouse(_tcscmp(pstrValue, _T("true")) == 0);
		}
		else if (_tcscmp(pstrName, _T("handcur")) == 0)
		{
			m_bHandCur = (_tcscmp(pstrValue, _T("true")) == 0);
		}
	...
	void CLabelUI::PaintText(HDC hDC)
	{
		...
		//#else
		//				USES_CONVERSION;
		//				std::wstring mTextValue = A2W(m_TextValue.GetData());
		//
		//				nRc.Offset(-1,0);
	...
	void CLabelUI::SetEnabledMouse(bool bMouseEvent)
	{
