...
namespace DuiLib
{
	...
	void CSliderUI::SetValue(int nValue) //2014.7.28    当鼠标正在滑动滑块时不会收到SetValue的影响，比如滑动改变音乐的进度，不会因为外部一直调用SetValue而让我们无法滑动滑块
	{
	...
	void CSliderUI::DoEvent(TEventUI& event)
	{
		...
		{
			if( IsEnabled() ) {//2014.7.28   注释掉原来的代码，加上这些代码后可以让Slider不是在鼠标弹起时才改变滑块的位置
...
// 		}
		if( event.Type == UIEVENT_BUTTONUP )
		{
			...
			if(/*m_nValue !=nValue && 2014.7.28   这个注释很关键，是他导致了鼠标拖动滑块无法发出DUI_MSGTYPE_VALUECHANGED消息*/nValue>=m_nMin && nValue<=m_nMax)
			{
				m_nValue =nValue;
		...
		{
			if( (m_uButtonState & UISTATE_CAPTURED) != 0 ) {//2014.7.28   重写这个消息判断让Slider发出DUI_MSGTYPE_VALUECHANGED_MOVE消息，让他在滑动过程也发出消息，比如用在改变音量时，一边滑动就可以一边改变音量
				if( m_bHorizontal ) {
...
// 		}
		...
		{//2014.7.28   只有鼠标在滑块的范围内才变为UISTATE_HOT
// 			if( IsEnabled() ) {
// 	  			m_uButtonState |= UISTATE_HOT;
// 				Invalidate();
// 			}
