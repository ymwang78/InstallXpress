...
namespace DuiLib {
	...
	VOID CUIAnimation::StopAnimation(int nAnimationID /*= 0*/)
	{
		...
		if(nAnimationID  != 0)
		{
			...
			if( NULL != pData )
			{
				if(m_pControl->GetManager()) m_pControl->GetManager()->KillTimer( m_pControl, nAnimationID );
		...
		else
		{
			...
			{
				if (m_pControl->GetManager()) m_pControl->GetManager()->KillTimer( m_pControl, m_arAnimations[i]->m_nAnimationID );
				delete m_arAnimations[i];
				m_arAnimations[i] = NULL;
			}
