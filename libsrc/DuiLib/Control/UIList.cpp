...
namespace DuiLib {
	...
	CListUI::CListUI() : m_pCallback(NULL), m_bScrollSelect(false), m_iCurSel(-1), m_iExpandedItem(-1)
	{
		...
		m_singlebkcolor = 0xFFF2FBFB;
		m_doublebkcolor = 0xFFCFE5F6;
		m_dwGribLineColor = 0xFFE4E4E4;
		m_bAlternatelybk = false;
		m_bVisibleGridLine = false;
		m_nItemHeight = 30;
	...
	void CListUI::SetPos(RECT rc)
	{
		...
		m_pList->SetScrollStepSize(m_nItemHeight);
	}
	...
	void CListUI::DoEvent(TEventUI& event)
	{
		...
		case UIEVENT_SCROLLWHEEL:
		{
			...
			case SB_LINEUP:
				...
				if (m_pManager != NULL) {
					m_pManager->SendNotify(this, DUI_MSGTYPE_SCROLL, 0, 0);
				}
				return;
			case SB_LINEDOWN:
	...
	bool CListUI::SelectItem(int iIndex, bool bTakeFocus, bool bsendmsg)
	{
	...
	void CListUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		else if (_tcscmp(pstrName, _T("alternatelybk")) == 0) {
			SetAlternatelyBk(_tcscmp(pstrValue, _T("true")) == 0);
		}
		else if (_tcscmp(pstrName, _T("singlebkcolor")) == 0) {
			if (*pstrValue == _T('#')) pstrValue = ::CharNext(pstrValue);
			LPTSTR pstr = NULL;
			DWORD clrColor = _tcstoul(pstrValue, &pstr, 16);
			SetSinglebkColor(clrColor);
		}
		else if (_tcscmp(pstrName, _T("doublebkcolor")) == 0) {
			if (*pstrValue == _T('#')) pstrValue = ::CharNext(pstrValue);
			LPTSTR pstr = NULL;
			DWORD clrColor = _tcstoul(pstrValue, &pstr, 16);
			SetDoublebkColor(clrColor);
		}
		else if (_tcscmp(pstrName, _T("visiblescroll")) == 0)
		{
			SetVisibleScroll(_tcscmp(pstrValue, _T("true")) == 0);
			return;
		}
		else if (_tcscmp(pstrName, _T("gridline")) == 0)
		{
			SetVisibleGridLine(_tcscmp(pstrValue, _T("true")) == 0);
		}
		else if (_tcscmp(pstrName, _T("gridlinecolor")) == 0)
		{
			if (*pstrValue == _T('#')) pstrValue = ::CharNext(pstrValue);
			LPTSTR pstr = NULL;
			DWORD clrColor = _tcstoul(pstrValue, &pstr, 16);
	...
	void CListUI::SetVisibleGridLine(bool bVisible)
	{
		m_bVisibleGridLine = bVisible;
		Invalidate();
	}

	bool CListUI::IsVisibleGridLine()
	{
		return m_bVisibleGridLine;
	}

	void CListUI::SetGridLineColor(DWORD dwColor)
	{
		if (m_dwGribLineColor == dwColor)
			return;
		m_dwGribLineColor = dwColor;
		Invalidate();
	}

	void CListUI::SetVisibleScroll(bool bvisible)
	{
		m_bVisibleScroll = bvisible;
		if (m_pList) m_pList->SetVisibleScroll(m_bVisibleScroll);
		if (m_pVerticalScrollBar) m_pVerticalScrollBar->SetEnabledDraw(m_bVisibleScroll);
		if (m_pHorizontalScrollBar) m_pHorizontalScrollBar->SetEnabledDraw(m_bVisibleScroll);
	}

	bool CListUI::GetVisibleScroll()
	{
		return m_bVisibleScroll;
	}

	void CListUI::SetSinglebkColor(DWORD dcolor)
	{
		m_singlebkcolor = dcolor;
		Invalidate();
	}

	void CListUI::SetDoublebkColor(DWORD dcolor)
	{
		m_doublebkcolor = dcolor;
		Invalidate();
	}

	void CListUI::SetItemHeight(int nheight)
	{
		m_nItemHeight = nheight;
	}

	int CListUI::GetItemHeight()
	{
		return m_nItemHeight;
	}

	void CListUI::SetAlternatelyBk(bool balternately)
	{
		m_bAlternatelybk = balternately;
	}

	void CListUI::PaintBkColor(HDC hDC)
	{
		__super::PaintBkColor(hDC);
		int nheight = m_rcItem.bottom - m_rcItem.top;
		if (m_bAlternatelybk && m_nItemHeight > 0 && nheight > 0 && m_pList)
		{
			RECT rect = m_rcItem;
			if (m_pHeader && m_pHeader->IsVisible())
				rect.top += m_pHeader->GetHeight();
			else
				rect.top += 0;

			int nstartdrawsize = m_nItemHeight - m_pList->GetScrollPos().cy%m_nItemHeight;
			int nRows = nheight / m_nItemHeight;
			if (nstartdrawsize > 0)
			{
				nRows = nRows + 1;
				rect.bottom = rect.top + nstartdrawsize;
			}

			for (int i = 0; rect.top < m_rcItem.bottom && i < nRows; i++)
			{				
				if (rect.bottom > m_rcItem.bottom)
					rect.bottom = m_rcItem.bottom;

				if (rect.top < m_rcItem.bottom)
				{
					if ((i + 1) % 2 != 0)
						CRenderEngine::DrawColor(hDC, rect, GetAdjustColor(m_singlebkcolor));
					else
						CRenderEngine::DrawColor(hDC, rect, GetAdjustColor(m_doublebkcolor));
				}
				rect.top = rect.bottom;
				rect.bottom += m_nItemHeight;
			}
		}
		if (m_items.GetSize() <= 3 && m_pHeader) 
			PaintGribLine(hDC);
	}

	void CListUI::PaintGribLine(HDC hDC)
	{
		if (!m_bVisibleGridLine)
			return;

		int nheight = m_rcItem.bottom - m_rcItem.top;
		if (m_nItemHeight > 0 && nheight > 0 && m_pList)
		{
			RECT rect = m_rcItem;
			if (m_pHeader && m_pHeader->IsVisible())
				rect.top += m_pHeader->GetHeight();
			...
			for (int i = 0; rect.top < m_rcItem.bottom && i < nRows; i++)
			{
				...
				if (rect.top < m_rcItem.bottom)
				{
					RECT rcGridline = rect;
					rcGridline.top = rect.top;
					rcGridline.bottom = rcGridline.top + 1;
					CRenderEngine::DrawColor(hDC, rcGridline, GetAdjustColor(m_dwGribLineColor));
				}
				rect.top += m_nItemHeight;
			}
		}

		if (m_ListInfo.nColumns > 0)
		{
			RECT rect = m_rcItem;
			for (int i = 0; i < m_ListInfo.nColumns; i++)
			{
				RECT rcGridline = m_ListInfo.rcColumn[i];
				rcGridline.left = rcGridline.right - 1;
				rcGridline.right = rcGridline.right;
				rcGridline.top = m_rcItem.top + (rcGridline.bottom - rcGridline.top);
				rcGridline.bottom = rcGridline.top + (m_rcItem.bottom - m_rcItem.top) - (rcGridline.bottom - rcGridline.top);
				CRenderEngine::DrawColor(hDC, rcGridline, GetAdjustColor(m_dwGribLineColor));
			}
		}		
	}

	void CListUI::SetPrePaintCallback(IItemPrePaintCallback* pCallback)
	{
		if (m_pList)
			m_pList->SetPrePaintCallback(pCallback);
	}
	...
	void CListBodyUI::SetVisibleScroll(bool bVisible)
	{
		m_bvisible = bVisible;
		__super::SetVisibleScroll(m_bvisible);
	}
	...
	void CListBodyUI::SetPrePaintCallback(IItemPrePaintCallback* pCallback)
	{
		m_pPrePaintCallback = pCallback;
	}
	...
	void CListHeaderItemUI::PaintStatusImage(HDC hDC)
	{
...

		{
			...
			else if (m_dwSepColor != 0xFF010101)
			{
				CDuiRect rect = m_rcItem;
				rect.left = rect.right - (rcThumb.right - rcThumb.left);
				CRenderEngine::DrawColor(hDC, rect, GetAdjustColor(m_dwSepColor));
			}
		}
	}
	...
	void CListElementUI::DrawItemBk(HDC hDC, const RECT& rcItem)
	{
		...
		if (iBackColor != 0) {
			...
			rcBkItem.top += GetPadding().top;
			rcBkItem.left += GetPadding().left;
			rcBkItem.right -= GetPadding().right;
			rcBkItem.bottom -= GetPadding().bottom;
			CRenderEngine::DrawColor(hDC, rcBkItem, GetAdjustColor(iBackColor));
		}
		...
		if (m_pOwner) m_pOwner->PaintGribLine(hDC);
	}
	...
	CListLabelElementUI::CListLabelElementUI()
		...
		, m_strLabelImage(_T(""))
	{
		m_rcTextPadding = CDuiRect(0, 0, 0, 0);
	}
	...
	void CListLabelElementUI::SetLabelImage(LPCTSTR pstrImage)
	{
		if (m_strLabelImage == pstrImage)
			return;

		m_strLabelImage = pstrImage;
		Invalidate();
	}

	void CListLabelElementUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		if (_tcscmp(pstrName, _T("deftextcolor")) == 0) {
			if (*pstrValue == _T('#')) pstrValue = ::CharNext(pstrValue);
			LPTSTR pstr = NULL;
			DWORD clrColor = _tcstoul(pstrValue, &pstr, 16);
			SetDefTextColor(clrColor);
		}
		else if (_tcscmp(pstrName, _T("labelimage")) == 0) {
			SetLabelImage(pstrValue);
		}
		else if (_tcscmp(pstrName, _T("textpadding")) == 0) {
			LPTSTR pstr = NULL;
	...
	void CListLabelElementUI::SetDefTextColor(DWORD dwColor)
	{
	...
	void CListLabelElementUI::DrawItemText(HDC hDC, const RECT& rcItem, bool bParent)
	{
		if (m_sText.IsEmpty()) return;

		if (m_pOwner == NULL) return;

		if (!m_strLabelImage.IsEmpty())
		{
			if (!DrawImage(hDC, m_strLabelImage.GetData()))
				m_strLabelImage.Empty();
		}
		...
		int nLinks = 0;
		RECT rcText = rcItem;
		rcText.left += (pInfo->rcTextPadding.left + m_rcTextPadding.left);
		rcText.right -= (pInfo->rcTextPadding.right + m_rcTextPadding.right);
		rcText.top += (pInfo->rcTextPadding.top + m_rcTextPadding.top);
		rcText.bottom -= (pInfo->rcTextPadding.bottom + m_rcTextPadding.bottom);
		...
		if (pInfo->bShowHtml) {
			...
			if (bParent || m_dwDefTextColor == 0) {
				CRenderEngine::DrawText(hDC, m_pManager, rcText, m_sText, iTextColor, \
					pInfo->nFont, DT_SINGLELINE | pInfo->uTextStyle);
			}
			else {
				CRenderEngine::DrawText(hDC, m_pManager, rcText, m_sText, m_dwDefTextColor, \
					pInfo->nFont, DT_SINGLELINE | pInfo->uTextStyle);
			}
		}
	}
	...
	void CListTextElementUI::DrawItemText(HDC hDC, const RECT& rcItem, bool bParent)
	{
		if (m_pOwner == NULL) return;
		TListInfoUI* pInfo = m_pOwner->GetListInfo();
		DWORD iTextColor = pInfo->dwTextColor;
		...
		for (int i = 0; i < pInfo->nColumns; i++)
		{
			...
			CDuiString strText = _T("");//不使用LPCTSTR，否则限制太多 by cddjr 2011/10/20
			UINT uTextStyle = pInfo->uTextStyle;
			if (pCallback) {
				strText = pCallback->GetItemText(this, m_iIndex, i, uTextStyle, iTextColor);
			}
			else {
			...
			if (pInfo->bShowHtml)
				...
				CRenderEngine::DrawText(hDC, m_pManager, rcItem, strText.GetData(), iTextColor, \
					pInfo->nFont, DT_SINGLELINE | uTextStyle);
	...
	CListContainerElementUI::CListContainerElementUI() :
		...
		m_uButtonState(0),
		m_bMouseEvent(false),
		m_bEnableDrawBk(true),
		m_bVisibleSendMsg(false)
	{
	}
	...
	void CListContainerElementUI::SetVisible(bool bVisible)
	{
		...
		if (bVisible && m_bVisibleSendMsg)
		{
			if (m_pManager != NULL) m_pManager->SendNotify(this, DUI_MSGTYPE_ITEMVISIBLE);
		}
	}

	void CListContainerElementUI::SetVisibleSendMsg(bool bSend)
	{
		m_bVisibleSendMsg = bSend;
	}
	...
	bool CListContainerElementUI::IsMouseEvent() const
	{
		return m_bMouseEvent;
	}

	void CListContainerElementUI::SetMouseEvent(bool bMouseEvent)
	{
		m_bMouseEvent = bMouseEvent;
	}

	void CListContainerElementUI::SetEnableDrawBk(bool bEnableDrawBk)
	{
		m_bEnableDrawBk = bEnableDrawBk;
	}
	...
	void CListContainerElementUI::DoEvent(TEventUI& event)
	{
		...
		if (event.Type == UIEVENT_MOUSEENTER)
		{
			if (IsEnabled()) {
				...
				if (m_pManager != NULL && m_bMouseEvent) {
	...
	{
		if (_tcscmp(pstrName, _T("selected")) == 0) Select();
		else if (_tcscmp(pstrName, _T("mouseevent")) == 0) SetMouseEvent(_tcscmp(pstrValue, _T("true")) == 0);
		else if (_tcscmp(pstrName, _T("enabledrawbk")) == 0) SetEnableDrawBk(_tcscmp(pstrValue, _T("true")) == 0);
		else CContainerUI::SetAttribute(pstrName, pstrValue);
	}
	...
	void CListContainerElementUI::DrawItemText(HDC hDC, const RECT& rcItem, bool bParent)
	{
		return;
	}
	...
	{
		ASSERT(m_pOwner);
		if (m_pOwner == NULL || m_bEnableDrawBk == false) return;
		TListInfoUI* pInfo = m_pOwner->GetListInfo();
		DWORD iBackColor = 0;
		if (!pInfo->bAlternateBk || m_iIndex % 2 == 0) iBackColor = pInfo->dwBkColor;
	...
	CListContainerElementExUI::CListContainerElementExUI()
		:m_pListCallBack(NULL)
	{
	}

	LPCTSTR CListContainerElementExUI::GetClass() const
	{
		return _T("ListContainerElementExUI");
	}

	UINT CListContainerElementExUI::GetControlFlags() const
	{
		return UIFLAG_WANTRETURN;
	}
	...
	void CListContainerElementExUI::DoEvent(TEventUI& event)
	{
		...
		if (m_pListCallBack)
		{
			m_pListCallBack->ListContainerDoEvent(this, event);
		}
	...
	void CListContainerElementExUI::SetICallBack(IlistContainerElementCall* pListCallBack)
	{
		m_pListCallBack = pListCallBack;
	}



	/////////////////////////////////////////////////////////////
	CListContainerElementComboUI::CListContainerElementComboUI()
		: m_pListCallBack(NULL)
	{

	}

	LPCTSTR CListContainerElementComboUI::GetClass() const
	{
		return _T("ListContainerElementComboUI");
	}

	UINT CListContainerElementComboUI::GetControlFlags() const
	{
		return UIFLAG_WANTRETURN;
	}

	LPVOID CListContainerElementComboUI::GetInterface(LPCTSTR pstrName)
	{
		if (_tcscmp(pstrName, DUI_CTR_LISTCONTAINERELEMENTCOMBO) == 0) return static_cast<CListContainerElementComboUI*>(this);
		return __super::GetInterface(pstrName);
	}

	void CListContainerElementComboUI::SetICallBack(IlistContainerElementCall* pListCallBack)
	{
		m_pListCallBack = pListCallBack;
	}

	void CListContainerElementComboUI::DoEvent(TEventUI& event)
	{
		if (!IsMouseEnabled() && event.Type > UIEVENT__MOUSEBEGIN && event.Type < UIEVENT__MOUSEEND) {
			if (m_pOwner != NULL) m_pOwner->DoEvent(event);
			else CContainerUI::DoEvent(event);
	...
	void CListContainerElementComboUI::SetPos(RECT rc)
	{
		CHorizontalLayoutUI::SetPos(rc);
	}

	void CListContainerElementComboUI::DoPaint(HDC hDC, const RECT& rcPaint)
	{
		if (!::IntersectRect(&m_rcPaint, &rcPaint, &m_rcItem)) return;
		DrawItemBk(hDC, m_rcItem);
		DrawItemText(hDC, m_rcItem, false);
		CContainerUI::DoPaint(hDC, rcPaint);
	}

	void CListContainerElementComboUI::DrawItemText(HDC hDC, const RECT& rcItem, bool bParent)
	{
		if (m_sText.IsEmpty()) return;
		...
		for (int i = 0; i < nCount; i++)
		{
			CHorizontalLayoutUI *pLayout = static_cast<CHorizontalLayoutUI*>(m_items[i]);
			for (int j = 0; pLayout && j < pLayout->GetCount(); j++)
			{
				CLabelUI *pLabel = static_cast<CLabelUI*>(pLayout->GetItemAt(i)->GetInterface(_T("Label")));
				if (pLabel)
				{
					pLabel->SetTextColor(iTextColor);
				}
			}
		}
	}
