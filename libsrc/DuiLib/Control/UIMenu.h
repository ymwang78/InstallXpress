...
enum MenuItemDefaultInfo
{
	...
	ITEM_DEFAULT_EXPLAND_ICON_WIDTH = 8,	//默认下级菜单扩展图标所占宽度
...
struct MenuData
{
	CDuiString strName;
	CDuiString strData;
	MenuData():strData(_T("")),strName(_T("")) {};
};
...
public:
	 ...
	 */
    void Init(CMenuElementUI* pOwner, STRINGorID xml, CDuiString xmltype, POINT point,
	...
	void Show(int w = 0, int h = 0);
	CPaintManagerUI* GetMenuPaintManager();
public:

	POINT			m_BasedPoint;
	STRINGorID		m_xml;
	CDuiString      m_xmltype;
...
public:
	...
	void SetCheckBkImage(LPCTSTR strimg);
	void SetCheckHotImage(LPCTSTR strimg);
	void SetShowExplandIcon(bool bShow);
	void DrawItemExpland(HDC hDC, const RECT& rcItem);
	void DrawItemCheckImage(HDC hDC, const RECT& rcItem);
	void SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue);
...
protected:
	...
	CDuiString	m_strCheckBkimage;
	CDuiString  m_strCheckHotimage;
	bool		m_bCheckItem;
