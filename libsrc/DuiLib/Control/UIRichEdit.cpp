...
void CRichEditUI::CharFormattoLogFont(const CHARFORMAT &cf, COLORREF &textcolor, LOGFONT &lf)
{
	CHARFORMAT2 cf2;
	cf2.cbSize = cf.cbSize;
	cf2.dwMask = cf.dwMask;
	cf2.dwEffects = cf.dwEffects;
	cf2.yHeight = cf.yHeight;
	cf2.yOffset = cf.yOffset;
	cf2.crTextColor = cf.crTextColor;
	cf2.bCharSet = cf.bCharSet;
	cf2.bPitchAndFamily = cf.bPitchAndFamily;
	wcscpy_s(cf2.szFaceName, LF_FACESIZE - 1, cf.szFaceName);
	CharFormat2LogFont(cf2, textcolor, lf);
}

void CRichEditUI::LogFont2CharFormat(const LOGFONT &lf, const COLORREF &textcolor, DWORD Flags, CHARFORMAT &cf)
{
	cf.dwEffects = 0;
	cf.dwMask = 0;
	...
	if ((Flags & CF_NOSTYLESEL) == 0) {
		...
		cf.dwEffects |= (lf.lfWeight == FW_BOLD ? TRUE : FALSE) ? CFE_BOLD : 0;
		cf.dwEffects |= (lf.lfItalic ? TRUE : FALSE) ? CFE_ITALIC : 0;
	}
	...
	if ((Flags & CF_NOSIZESEL) == 0) {
		...
		LONG yPerInch = ::GetDeviceCaps(hDispDC,LOGPIXELSY);
		cf.yHeight = -(int)(lf.lfHeight * 1440 / yPerInch);
		DeleteDC(hDispDC);
	}
	...
	if ((Flags & CF_NOFACESEL) == 0) {
		cf.dwMask |= CFM_FACE;
		cf.bPitchAndFamily = lf.lfPitchAndFamily;
		wcscpy_s(cf.szFaceName, LF_FACESIZE - 1, lf.lfFaceName);
	}

	if (Flags & CF_EFFECTS) {
		cf.dwMask |= CFM_UNDERLINE | CFM_STRIKEOUT | CFM_COLOR;
		cf.dwEffects |= (lf.lfUnderline ? TRUE : FALSE) ? CFE_UNDERLINE : 0;
		cf.dwEffects |= (lf.lfStrikeOut ? TRUE : FALSE) ? CFE_STRIKEOUT : 0;
	...
	//BOOL bIsBold = (lf.lfWeight>=FW_BOLD);
...
void CRichEditUI::CharFormat2LogFont(const CHARFORMAT2 &cf, COLORREF &textcolor, LOGFONT &lf)
{
	DWORD dwFlags = 0;
	...
	if (bIsFormat2)
	{
		...
		if (pCF2->dwMask & CFM_WEIGHT)
			lf.lfWeight = pCF2->wWeight;

		if (cf.dwMask & CFM_FACE)
		{
			lf.lfPitchAndFamily = pCF2->bPitchAndFamily;
			wcscpy_s(lf.lfFaceName, LF_FACESIZE - 1, pCF2->szFaceName);
		}
	}
	else
	{
		lf.lfPitchAndFamily = cf.bPitchAndFamily;
		wcscpy_s(lf.lfFaceName, LF_FACESIZE - 1, cf.szFaceName);
	}
	...
	//swprintf(lf.lfFaceName, cf.szFaceName);

	//textcolor = cf.crTextColor;
}

void CRichEditUI::CharFormattoCharFormat2(const CHARFORMAT &cf, CHARFORMAT2 &cf2)
{
	cf2.cbSize = cf.cbSize;
	cf2.dwMask = cf.dwMask;
	cf2.dwEffects = cf.dwEffects;
