...
LPCTSTR CControlUI::GetBkCoverImage()
{
	return m_sBkCoverImage;
}

void CControlUI::SetBkCoverImage(LPCTSTR pStrImage)
{
	if (m_sBkCoverImage == pStrImage) return;

	m_sBkCoverImage = pStrImage;
	Invalidate();
}
...
bool CControlUI::CenterDrawImage(HDC hDC, LPCTSTR pStrImage, LPCTSTR pStrModify)
{
	return CRenderEngine::CenterDrawImageString(hDC, m_pManager, m_rcItem, m_rcPaint, pStrImage, pStrModify);
}
...
UINT_PTR CControlUI::GetBelongTag() const
{
	return m_pBelongTag;
}

void CControlUI::SetBelongTag(UINT_PTR pTag)
{
	m_pBelongTag = pTag;
}
...
void CControlUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	...
	else if( _tcscmp(pstrName, _T("autoicnheight")) == 0) SetAutoIncHeight(_ttoi(pstrValue));
}

void CControlUI::SetAutoIncHeight(int iHeight)
{
	m_iAutoIncHeight = iHeight;
}

int CControlUI::GetAutoIncHeight() const
{
	return m_iAutoIncHeight <= 1 ? 0 : m_iAutoIncHeight;
}
...
void CControlUI::PaintBkConverImage(HDC hDC)
{
	return;
}
...
void CControlUI::PaintBorder(HDC hDC)
{
	if(m_dwBorderColor != 0 || m_dwFocusBorderColor != 0)
	{
		...
		{
			if (m_bhot && m_dwHotBorderColor !=0 && m_nBorderSize > 0)
				CRenderEngine::DrawRect(hDC, m_rcItem, m_nBorderSize, GetAdjustColor(m_dwHotBorderColor));
			else if (IsFocused() && m_dwFocusBorderColor != 0 && m_nBorderSize > 0)
