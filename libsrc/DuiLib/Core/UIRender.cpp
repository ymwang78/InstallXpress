...
TImageInfo* CRenderEngine::LoadImage(STRINGorID bitmap, LPCTSTR type, DWORD mask)
{
	...
	if (*(int*)pData == *(int*)"RIFF" && *(int*)(pData + 8) == *(int*)"WEBP") {
		hBitmap = WebpLoad(pData, dwSize, x, y, bAlphaChannel, mask);
	}
	else {
		hBitmap = StbiLoad(pData, dwSize, x, y, bAlphaChannel, mask);
	}
...
HBITMAP CRenderEngine::WebpLoad(LPBYTE pData, DWORD dwSize,
	int& x, int& y, bool& bAlphaChannel, DWORD mask)
{
#if SUPPORT_WEBP
	int ret = WebPGetInfo((const uint8_t*)pData, dwSize, &x, &y);
	...
	uint8_t* pImage = WebPDecodeRGBA((const uint8_t*) pData, dwSize, &x, &y);
...
HBITMAP CRenderEngine::StbiLoad(LPBYTE pData, DWORD dwSize,
	int& x, int& y, bool& bAlphaChannel, DWORD mask)
{
	LPBYTE pImage = NULL;
	int n;
...
void CRenderEngine::DrawAlphaBitBlt(HDC hDC, int nDestX, int nDestY, int dwWidth, int dwHeight, HDC hSrcDC, int nSrcX, int nSrcY, int wSrc, int hSrc, BYTE uFade)
{
	...
	lpAlphaBlend(hDC, nDestX, nDestY,dwWidth, dwHeight, hSrcDC,nSrcX, nSrcY, wSrc, hSrc, bf);
}
...
bool CenterDrawImage(HDC hDC, CPaintManagerUI* pManager, const RECT& rc, const RECT& rcPaint, const CDuiString& sImageName, \
	const CDuiString& sImageResType, RECT rcItem, RECT rcBmpPart, RECT rcCorner, DWORD dwMask, BYTE bFade, \
	bool bHole, bool bTiledX, bool bTiledY)
{
	if (sImageName.IsEmpty()) {
	...
	if (rcBmpPart.right > 0)
	{
		if ((rc.right - rc.left) / 2 - (rcBmpPart.right - rcBmpPart.left) / 2 > 0)
		{
			rcItem.left += (rc.right - rc.left) / 2 - (rcBmpPart.right - rcBmpPart.left) / 2;
			rcItem.right = rcItem.left + (rcBmpPart.right - rcBmpPart.left);
		}
	}
	else
	{
		if ((rc.right - rc.left) / 2 - data->nX / 2 > 0)
		{
			rcItem.left += (rc.right - rc.left) / 2 - data->nX / 2;
			rcItem.right = rcItem.left + data->nX;
		}
	}	
	if (rcBmpPart.bottom > 0)
	{
		if ((rc.bottom - rc.top) / 2 - (rcBmpPart.bottom - rcBmpPart.top) / 2 > 0)
		{
			rcItem.top += (rc.bottom - rc.top) / 2 - (rcBmpPart.bottom - rcBmpPart.top) / 2;
			rcItem.bottom = rcItem.top + (rcBmpPart.bottom - rcBmpPart.top);
		}
	}
	else
	{
		if ((rc.bottom - rc.top) / 2 - data->nY / 2 > 0)
		{
			rcItem.top += (rc.bottom - rc.top) / 2 - data->nY / 2;
...
bool CRenderEngine::DDrawImage(HDC hDC, CPaintManagerUI* pManager, const RECT& rc, const RECT& rcPaint, const CDuiString& sImageName, \
	const CDuiString& sImageResType, RECT rcItem, RECT rcBmpPart, RECT rcCorner, DWORD dwMask, BYTE bFade, \
	bool bHole, bool bTiledX, bool bTiledY)
{
    ...
    for( int i = 0; i < 2; ++i,image_count = 0 ) {
        ...
        while( *pStrImage != _T('\0') ) {
            ...
            if( !sValue.IsEmpty() ) {
				...
				else if (sItem == _T("indent")) {
					rcItem.left += _tcstol(sValue.GetData(), &pstr, 10);  ASSERT(pstr);
					rcItem.top += _tcstol(pstr + 1, &pstr, 10);    ASSERT(pstr);
					rcItem.right -= _tcstol(pstr + 1, &pstr, 10);  ASSERT(pstr);
					if (rcItem.right < rcItem.left) rcItem.right = rc.left;
					rcItem.bottom -= _tcstol(pstr + 1, &pstr, 10); ASSERT(pstr);
					if (rcItem.bottom < rcItem.top) rcItem.bottom = rc.bottom;
				}
...
bool CRenderEngine::DrawImageString(HDC hDC, CPaintManagerUI* pManager, const RECT& rc, const RECT& rcPaint,
	LPCTSTR pStrImage, int ndestx, int ndesty, const RECT& rcSource, LPCTSTR pStrModify)
{
	if ((pManager == NULL) || (hDC == NULL)) return false;

	// 1、aaa.jpg
	// 2、file='aaa.jpg' res='' restype='0' dest='0,0,0,0' source='0,0,0,0' corner='0,0,0,0' indent='0,0,0,0'
	// mask='#FF0000' fade='255' hole='false' xtiled='false' ytiled='false'
	...
	if (ndestx != 0)
	{	
		rcItem.left = rcItem.left + ndestx;
		rcItem.right = rcItem.right + ndestx;		
	}

	if (ndesty != 0)
	{		
		rcItem.top = rcItem.top + ndesty;
		rcItem.bottom = rcItem.bottom + ndesty;	
	}

	if (rcSource.left > 0 || rcSource.right > 0 || rcSource.top > 0 || rcSource.bottom > 0)
	{
		rcBmpPart.left = rcSource.left;
		rcBmpPart.right = rcSource.right;
		rcBmpPart.top = rcSource.top;
		rcBmpPart.bottom = rcSource.bottom;
	}
...
bool CRenderEngine::CenterDrawImageString(HDC hDC, CPaintManagerUI* pManager, const RECT& rc, const RECT& rcPaint,
	LPCTSTR pStrImage, LPCTSTR pStrModify)
{
	if ((pManager == NULL) || (hDC == NULL)) return false;
	...
	for (int i = 0; i < 2; ++i, image_count = 0) {
		...
		while (*pStrImage != _T('\0')) {
			...
			if (!sValue.IsEmpty()) {
				if (sItem == _T("file") || sItem == _T("res")) {
					if (image_count > 0)
						DuiLib::CenterDrawImage(hDC, pManager, rc, rcPaint, sImageName, sImageResType,
							rcItem, rcBmpPart, rcCorner, dwMask, bFade, bHole, bTiledX, bTiledY);
...
CDuiString CRenderEngine::GetLimitText(HDC hDC, CPaintManagerUI* pManager, LPCTSTR pstrText, int iFont, int iLimitLen)
{
	CDuiString strTemp = pstrText;
	int textLen = GetTextSize(hDC, pManager, pstrText, iFont, 0).cx;
	if (textLen <= iLimitLen) return strTemp;
	int iTempLen = GetTextSize(hDC, pManager, _T("..."), iFont, 0).cx;
	if (iTempLen >= iLimitLen) return _T("");
	CDuiString sRes = _T("");
	int i = strTemp.GetLength() - 1;
	while (textLen > iLimitLen - iTempLen)
	{
		if (strTemp.GetAt(i) == 0)
			break;
		else if (strTemp.GetAt(i) > 127)
			i -= 2;
		else
			i--;
		sRes = strTemp.Left(i);
		textLen = GetTextSize(hDC, pManager, sRes.GetData(), iFont, 0).cx;
	}
	return sRes + _T("...");
}
...
CTextToImage::CTextToImage(CDuiString strText, CDuiString pFontName /*= "宋体"*/, float fontSize/*=12*/, DWORD pColor/*=0x000000FF*/, int fontStyle/*=0*/)
{
	m_pFontColor = new Gdiplus::Color(Gdiplus::Color::Black);
	m_pStrText = strText;
...
void CTextToImage::SetString(CDuiString &strText)
{
	if (NULL == m_pStrText)
		return;
	m_pStrText = strText;
...
void CTextToImage::SaveToImage(HWND hWnd, CDuiString &strPicPath)
{
	...
	graphics.MeasureString(m_pStrText.GetData(), -1, &font, Gdiplus::PointF(0, 0), &boundingBox);
	...
	//【3】绘制文本
	g.DrawString(m_pStrText.GetData(), -1, &font, Gdiplus::PointF(0, 0), &solidBrush);

	CLSID   clsID = GUID_NULL;
	wchar_t ext[_MAX_EXT] = { 0 };
	wchar_t format[_MAX_EXT] = { L"image/" };

	_wsplitpath_s(strPicPath.GetData(), NULL, 0, NULL, 0, NULL, 0, ext, _MAX_EXT);
	wcscat_s(format, _MAX_EXT, ext + 1);
	if (GetEncoderClsid(format, &clsID))
	{
		bitmap.Save(strPicPath.GetData(), &clsID, NULL);
	}
}

void CTextToImage::DrawText(HWND hWnd, HDC hDC, int xpos, int ypos)
{
	if (NULL == m_pStrText || NULL == m_pFontColor)
		return;
	...
	//创建单个ItemDc(绘制Item);
	HDC hCloneDc = ::CreateCompatibleDC(hDC);
	HBITMAP hItemOldBitmap = (HBITMAP) ::SelectObject(hCloneDc, hbitmap);
	if (hCloneDc)
	{
		typedef BOOL(WINAPI *LPALPHABLEND)(HDC, int, int, int, int, HDC, int, int, int, int, BLENDFUNCTION);
		static LPALPHABLEND lpAlphaBlend = (LPALPHABLEND) ::GetProcAddress(::GetModuleHandle(_T("msimg32.dll")), "AlphaBlend");
		...
		if (lpAlphaBlend == NULL) lpAlphaBlend = AlphaBitBlt;
		lpAlphaBlend(hDC, xpos, ypos, (int)size.Width, (int)size.Height, hCloneDc, 0, 0, (int)size.Width, (int)size.Height, bf);

		::SelectObject(hCloneDc, hItemOldBitmap);
		::DeleteObject(hbitmap);
		::DeleteDC(hCloneDc);
	}
	else
...
void CTextToImage::SetFontName(CDuiString &fontName)
{
