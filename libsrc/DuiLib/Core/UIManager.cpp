...
bool CPaintManagerUI::m_bMessageloop = false;
...
void CPaintManagerUI::Finalizer()
{
	RemoveAllImages();
}
...
void CPaintManagerUI::SetTransParentHwnd(HWND hwnd)
{
	m_TransParenthwnd = hwnd;
}
    ...
    case WM_CLOSE:
        {
			...
			if (hwndParent != NULL && lParam >= 0) ::SetFocus(hwndParent);
        }
        break;
    case WM_ERASEBKGND:
        {
   ...
   case WM_PAINT:
	   {
		   ...
		   if(m_bAlphaBackground)
		   {
			   ...
			   RECT rcWnd = {0};
//             HWND hParent = ::GetParent(m_hWndPaint);
//             HWND hParent2 = (HWND)::GetWindowLong(m_hWndPaint, GWL_HWNDPARENT);
               if (m_TransParenthwnd != NULL) {
                   ::GetWindowRect(m_hWndPaint, &rcWnd);
                   RECT rcParentWnd = { 0 };
                   ::GetWindowRect(m_TransParenthwnd, &rcParentWnd);
                   rcWnd.left -= rcParentWnd.left;
                   rcWnd.top -= rcParentWnd.top;
               }
               else {
                   ::GetWindowRect(m_hWndPaint, &rcWnd);
               }
			   ...
			   ::UpdateLayeredWindow(m_hWndPaint, NULL, &pt,
                   &szWindow, m_hDcBackground,
...
CDuiString CPaintManagerUI::GetLimitText(LPCTSTR ptext, int nFont, int nMaxlen)
{
	return CRenderEngine::GetLimitText(GetPaintDC(), this, ptext, nFont, nMaxlen);
}
