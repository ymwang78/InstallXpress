...
class /*UTILITY_API*/ CMarkupStl
{
public:
	CMarkupStl() { x_InitMarkup(); SetDoc( NULL ); };
	CMarkupStl( MCD_CSTR szDoc ) { x_InitMarkup(); SetDoc( szDoc ); };
	CMarkupStl( int nFlags ) { x_InitMarkup(); SetDoc( NULL ); m_nDocFlags = nFlags; };
	CMarkupStl( const CMarkupStl& markup ) { x_InitMarkup(); *this = markup; };
