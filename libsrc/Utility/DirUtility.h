...
public:
	...
	//检测图片是否有效;
	static bool CheckFileInvalid(const std::string &strFilePath);

	//获取图片扩展名;
    static std::string GetFileSteamExtenName(unsigned char* pImagedata);
	...
	//获取appdata目录------vxim;
    static std::string GetAppDataChatDir();

	//创建多级目录;
    static void CreateMulitDir(const std::wstring& strdir);
	...
	//判断是否是文件夹;
    static bool IsDir(std::string szDir);
